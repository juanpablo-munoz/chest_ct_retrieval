...
def run_microf1_training(cfg, cuda_available, cuda_device=None, optimized_loaders=True, jupyter_mode=None):
    """Run micro-F1 based training pipeline"""
    from training.data_setup_local import (load_dataset_microf1, create_loaders_microf1, 
                                          create_loaders_microf1_optimized, create_loaders_microf1_standalone)
    ...
    if jupyter_mode is None:
        try:
            ...
            get_ipython()
            jupyter_mode = True
            print("Detected Jupyter environment - using single-threaded data loading")
        except NameError:
            jupyter_mode = False
            print("Detected standalone Python - enabling multiprocessing")
    ...
    # Create loaders based on environment and preferences
    if optimized_loaders:
        if jupyter_mode:
            loaders = create_loaders_microf1_optimized(
                train_set, test_set, cfg["training"]["batch"]["batch_size"],
                cuda_available, device=cuda_device, jupyter_friendly=True
            )
        else:
            loaders = create_loaders_microf1_standalone(
                train_set, test_set, cfg["training"]["batch"]["batch_size"],
                cuda_available, device=cuda_device
            )
    else:
        loaders = create_loaders_microf1(
            train_set, test_set, cfg["training"]["batch"]["batch_size"],
            cuda_available, gpu_preprocessing=True, device=cuda_device
        )
...
def main(training_mode="triplet", config_path="config/base.yaml", optimized_loaders=True, jupyter_mode=None):
    ...
    if training_mode == "triplet":
        ...
        trainer = run_microf1_training(cfg, cuda_available, cuda_device, optimized_loaders, jupyter_mode)
    else:
